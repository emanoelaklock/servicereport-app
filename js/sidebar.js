...
function renderSidebar(paginaAtiva) {
  ...
  const SVG = {
    ...
    devolvidas:    `<svg viewBox="0 0 24 24"><path d="M9 14 4 9l5-5"/><path d="M4 9h11a5 5 0 0 1 0 10H8"/></svg>`,
  ...
  const a = paginaAtiva
  // "Devolvidas" é a lista de Tarefas filtrada por status=devolvida (tarefa.html?f=devolvida) —
  // sub-item de Tarefas. Estado ativo detectado pela URL (sem precisar mudar as páginas).
  const onDevolvidas = a === 'tarefas' && /[?&]f=devolvida/.test(location.search || '')
  const item = (key, href, label, cor, active) =>
    `<a class="nav c-${cor}${(active === undefined ? a === key : active) ? ' on' : ''}" href="${href}"><span class="ni">${SVG[key]}</span>${label}</a>`
...
<aside class="side">
  ...
  ${item('tarefas', 'tarefa.html', 'Tarefas', 'green', a === 'tarefas' && !onDevolvidas)}
  <a class="nav c-red${onDevolvidas ? ' on' : ''}" href="tarefa.html?f=devolvida" style="margin-left:14px"><span class="ni">${SVG.devolvidas}</span>Devolvidas</a>
