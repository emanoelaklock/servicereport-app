-- 0078: a Jornada passa a contar o DESLOCAMENTO DO DIA (ida/retorno dentro da RAT).
-- Antes a vw_participacoes_dia só tinha branch de RAT (execução) e de viagem/pernoite (trechos),
-- então a ida/retorno do dia não virava chip nem entrava nas horas → a Jornada subcontava o
-- tempo (ex.: 13h46 em vez de 16h35). O §8.1 manda: tempo = execução + ida + retorno − almoço −
-- pausa. Aqui só ACRESCENTA os intervalos de ida/retorno; quem soma é a Jornada via uniaoMin
-- (união de intervalos), que dedup sozinha — o MESMO trecho lançado em várias RATs do dia conta
-- UMA vez (testado: ida repetida em 3 RATs não muda o total). Horas são HH:MM locais (sem fuso).
...
union all
 select rt.tecnico_id,
    ...
    'Ida'::text as referencia, r.rat_seq, r.cliente_id,
...
union all
 select rt.tecnico_id,
    coalesce(fn_date_ou_null(r.respostas ->> 'data'::text), r.data_tarefa::date) as dia,
    'desloc_dia'::text as artefato_tipo, r.id as artefato_id,
    'Retorno'::text as referencia, r.rat_seq, r.cliente_id,
