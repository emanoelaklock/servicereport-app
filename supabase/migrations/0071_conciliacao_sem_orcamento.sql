-- Conciliação: separa "Fora da proposta" (item usado fora de uma proposta QUE EXISTE) de
-- "Sem orçamento" (a Tarefa não tem proposta nenhuma — avulsa). Antes, qualquer material usado
-- numa tarefa sem orçamento caía como 'sem_orcada' (alarme vermelho). Agora:
--   · Tarefa COM proposta + item orçada=0 usado  -> 'sem_orcada'    (Fora da proposta, vermelho)
--   · Tarefa SEM proposta + item usado           -> 'sem_orcamento' (neutro; segue como pendência)
-- "tem proposta" = orcamento_id preenchido OU algum item com qtd_orcada>0. Resto IGUAL ao 0054.
create or replace view public.vw_conciliacao_tarefa as
 ...
 SELECT COALESCE(p.tarefa_id, u.tarefa_id) AS tarefa_id,
    ...
    COALESCE(p.qtd_levada, 0::numeric) - ceil(COALESCE(u.qtd_utilizada, 0::numeric)) AS qtd_devolvida,
        CASE
            WHEN COALESCE(p.qtd_orcada, 0::numeric) = 0::numeric AND ceil(COALESCE(u.qtd_utilizada, 0::numeric)) > 0::numeric
              THEN CASE WHEN COALESCE(pr.tem_proposta, false) THEN 'sem_orcada'::text ELSE 'sem_orcamento'::text END
...
-- DOWN: restaurar a view do 0054 (CASE sem o split sem_orcamento).
