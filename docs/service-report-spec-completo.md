...
- **Corte do go-live no servidor:** `desempenho_config.inicio` (**NULL = painel desligado — estado atual**); os RPCs e o snapshot recusam meses anteriores — retroativo nunca entra no placar. **Pré-requisito técnico CUMPRIDO (14/07): o teste do Pablo confirmou a estabilidade do encerramento no Android real da frota** (fixes v559→v575; a produção já está bem adiante). **Resta uma única chave: a data de go-live — decisão da gestão, um clique na página Desempenho.** Carência = início + 28 dias (selo no card via `desempenho_status()`).
