...
12. **Índice de assertividade do técnico — ✅ FEITO (08/26, SW v712):** seção **"Assertividade de preenchimento"** na página Desempenho (admin/gestor), por mês: % das RATs do técnico (data declarada; RAT do titular) **sem correção da gestão por `esquecimento_tecnico`** — o motivo já sai categorizado do editor auditado (`rat_edicoes`). Completação/correção de texto/mudança de processo **não contam contra** (aparecem como "outras edições"); `sync_app*` (re-sync do aparelho) fica fora. Tabela com avatar, campos mais esquecidos em chips (ex.: "Hora de Término ×3"), faixa de cor (≥90% verde · ≥70% âmbar · abaixo vermelho) e **drill por linha** (quando · RAT · campo · quem corrigiu). Validado contra produção (julho: 45 RATs/93% Pablo · 67% Charles · 62% Marcelo · 4 técnicos em 100% — números conferidos por SQL).
