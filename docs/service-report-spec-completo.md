...
- **Referência oficial de pré-lançamento:** snapshot de **julho/2026 carimbado 14/07 13:56** persistido em `desempenho_snapshots` (visível só à gestão via RLS; os RPCs do placar não leem snapshots — o corte do go-live segue intacto). Retrato: Alessandro 100 · Max 77 · Charles 75 · Maicon 68 · Arian 66 · Marcelo 65 · Pablo 59 · Francisco 58.
...
- **PÁGINA DESEMPENHO SEM PONTOS (14/07, SW v619/v620):** a página do portal foi virada pra leitura **"% de RATs COM problema"** (KPI da equipe, coluna "Resultado do mês", ordenação "Mais atenção" = maior % primeiro, tendência com **subir = vermelho**, sempre em "pontos percentuais" por extenso — comparação, não pontuação). Colunas "Encerramento das RATs" e "Problemas encontrados" **fundidas em "Encerramentos e ocorrências"** (linha neutra de encerramentos + chips de problema; o atraso aparece SÓ como chip, sem duplicar; tooltips explicam atribuição coletiva/individual). **Nenhum ponto, desconto ou nota aparece na interface**: RATs avaliadas mostram situação por extenso ("Com problema | Encerrada com atraso" / "Sem problema | Encerrada no dia" / "Não avaliada"); reedições mostram só data·campo·alteração·origem + chip "Reedição posterior"; devoluções mostram "RAT com problema" (sem −pts); o bloco "Dados técnicos da régua (auditoria)" **saiu da tela**. O **Índice interno de disciplina segue intacto no banco/snapshots** (cálculo, RPCs e histórico); se a auditoria precisar da leitura, sai por consulta/exportação administrativa — não pela página. O chip D+1 perdeu o "(meio ponto)". *(Isso substitui, NA UI, o item (a)/(d) da decisão consolidada acima — os formatos "XX/100" e "Entender minha nota com pesos" valem pro dado interno, não pra superfície.)*
