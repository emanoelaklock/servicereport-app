...
- **RAT de outra conta no aparelho (07/26):** aparelho que trocou de login pode ter RAT pendente criada sob a conta anterior — o RLS do servidor recusa (`42501`, só o dono grava a própria RAT) e antes ela re-tentava para sempre ("1 item com erro de envio" a cada sync, sem solução). Agora o sync reconhece a recusa de RLS, marca `envio_bloqueado_rls` e **para de re-tentar sob o login errado**; a lista de RATs rotula *"não enviada — RAT de {técnico}; entre com essa conta para enviar"* e o toast de erro não conta o item. Quando a conta dona loga, volta à fila sozinha (e o flag limpa no envio). Nada é apagado (regra abaixo) — excluir continua sendo ação explícita no 🗑.
