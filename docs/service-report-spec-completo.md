...
### 4.2 Formulário do pré-orçamento (reuso do layout da RAT) — *a construir*

Reaproveitar a **casca da RAT** (técnico já conhece): **Cliente (selecionável** do catálogo Omie, não fixo) · Data · Veículos · **Produtos** · Fotos · **Deslocamento** · **Pausa/Almoço** · **Técnicos responsáveis** · **Hora de início/término** · **Serviço solicitado** (campo amplo) · Observações. Diferença de **significado** (não é "o que fiz", é "o que vai precisar"):

- **Serviço solicitado** (o que o cliente quer) no lugar de "serviço executado". Campo de texto **amplo** (cabe descrição longa).
- **Produtos = material ESTIMADO** pra cotar (não consumido) — mesmo modal de Produtos.
- **DOIS tempos, ambos importam:**
  1. **Tempo da visita de levantamento** — botões **Iniciar / Encerrar visita** que **carimbam a hora automática** do relógio (igual ao timer da RAT; técnico não digita). Esse tempo conta na **jornada** do técnico (trabalho real) **e entra no valor do orçamento** (a visita é cobrada).
  2. **Estimativa de execução** — **campo OBRIGATÓRIO**, logo após "Serviço solicitado": quanto a obra vai levar, em **dias × equipe**. **A unidade de cobrança é hora-DUPLA** (2 técnicos = 1 dupla). Ex.: 2 dias (≈18h de obra) × 4 técnicos = 2 duplas → **36 horas-dupla**. É a base pra Thaís precificar a execução. (UI: dias + nº técnicos → app deriva as horas-dupla.)
- **Cor do artefato:** **amarelo** (`#F7B81E`) — RAT é verde, pré-orçamento é amarelo. (Convenção de cor por artefato.)
- **Ao finalizar:** PDF + e-mail pro comercial + **aparece no comercial-app, na lista "Pré-orçamento"** (o técnico cria no app do técnico → ao finalizar, entra pro comercial/Thaís precificar e virar Orçamento).

> *Status: especificado, a construir. É feature nova (não bloqueava o go-live do fluxo do técnico) — boa pra construir pós-go-live, com o app já rodando.*
...
- **Concluir o serviço (na Tarefa):** ação **deliberada e separada**, no nível da Tarefa. É **só aqui** que o serviço termina. Fluxo em **dois passos** pra não confundir: (1) "O serviço terminou?" → **Não, continua** (Tarefa segue "Atendimento continua", terá mais RATs) · **Sim, terminou**; (2) só se "Sim": **Sem pendência** → Tarefa "Concluída" · **Com pendência** → Tarefa "Concluída com pendência" (trabalho **encerrado e faturável**, com ressalva — ex.: serviço executado mas faltou levar plaqueta de fibra / patch cord; **não** é "volto pra continuar o serviço", que é "Atendimento continua"). A tarefa **fecha** (libera faturamento) e a pendência vira **alerta pro escritório** + botão **"Nova tarefa da pendência"** na tarefa concluída, que gera uma **tarefa de acabamento** vinculada à original (cliente/local/contrato herdados, descrição da pendência pré-preenchida) seguindo o fluxo normal. Concluir dispara o **documento consolidado**. Feita uma vez, pelo técnico em campo ou pelo admin. **"Concluída / Concluída c/ pendência" são estados da Tarefa, nunca da RAT diária** (a RAT só mostra "registrado ✓"). Botões na Tarefa: "RAT de hoje" (primário) e "Concluir serviço" (secundário, abre o fluxo de 2 passos). Referência visual: `docs/mockups/mockup-tarefa-multidia-app.html`.
...
- **Checkpoint de passagem (handoff entre idas) — resolve o "ninguém sabe onde parou / o que levar".** O gatilho NÃO é "marcou continua" (encheria o saco nos dias consecutivos). Ao encerrar a RAT, o app pergunta na língua do técnico: **"Você volta amanhã nesta tarefa?"** → **Sim, volto amanhã** = fecha o dia, sem mais nada · **Não, parei por agora** = abre **checkpoint OBRIGATÓRIO**: "o que ainda falta fazer aqui?" + "o que precisa levar pra terminar?". O checkpoint fica **no topo da Tarefa**, sempre mostrando a **última passagem** (quem/quando + falta + levar), visível pra quem retomar (mesma equipe ou outra) — antes mesmo de abrir a "RAT de hoje". Rede de segurança: tarefa parada há X dias sem RAT nova → cutuca "registre a passagem ou reagende".
  - **Nível 1 (go-live):** "o que falta" e "o que levar" em **texto livre**.
  - **Nível 2 (evolução pós-go-live):** "o que levar" vira **lista estruturada de material** (modal de Produtos) que alimenta a **lista de separação do almoxarifado (Francisco)** pra próxima ida; e "o que falta" pode usar progresso (ex.: "5 de 17 pontos") quando a tarefa tiver escopo quantificável.
...
> **Correção (multidia, §225):** "Concluída com pendência" **não é status da RAT**. Encerrar a RAT fecha o **dia** → `registrado`; concluir o serviço **com ou sem pendência** é ação **deliberada na Tarefa** (fluxo de 2 passos do botão "Concluir serviço"). A RAT nunca conclui o serviço.

1. **`Concluída com pendência`** — **status da TAREFA, definido pelo técnico (ou admin)** ao **concluir o serviço** na Tarefa (fluxo de 2 passos). O trabalho terminou, sobrou um **detalhe pequeno**. **NÃO bloqueia** faturamento nem fechamento da OS — a Tarefa fecha (libera faturamento) e a pendência vira **alerta pro escritório** + botão **"Nova tarefa da pendência"** (gera tarefa de acabamento vinculada à original).
...
- **Tempo no local é OBRIGATÓRIO na improdutiva** (cliente é cobrado por **deslocamento + permanência**): bloco "Tempo no local" (chegada/saída, calcula a duração); pré-preenche do timer se foi aberto, senão vazio; sempre editável. "Registrar visita" exige **motivo + tempo no local** (saída ≥ chegada, não no futuro). Substitui a regra antiga de "encerrar o atendimento antes" — o que importa é ter o tempo no local, tendo o timer sido aberto ou não.
