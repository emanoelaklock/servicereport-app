...
>
> **Última atualização: 27/07/2026** (incorpora o trabalho de 09–11/06: re-skin do admin, reorganização da RAT do técnico, timers reabríveis, IA "Melhorar escrita", pernoite, paleta oficial — **+ decisões de design de 12/06:** tempo por técnico + Tangerino (§8) · Remessa/Container (§9) · decimais com teto na Tarefa (§9) · Viagem como referência futura (§4.1) — **+ junho/26:** push de atribuição/reagendamento, telas de RAT/Deslocamento no portal, rótulo "Atendimento Realizado", sync resiliente, e **finalização colaborativa da viagem** — qualquer um a bordo finaliza, merge por união com conflito marcado pro admin (§4.1 / §12) — **+ julho/26:** usuário desativado continua visível no histórico com chip/avatar cinza "Inativo"; pickers de trabalho novo só com ativos; desligamento = desmarcar "Ativo" no Portal, sem remover `portal_acessos` (§12)).
...
- **Campos:** cliente (catálogo Omie) · descrição do que precisa ser feito (o levantamento) · **materiais necessários** (itens do catálogo, com quantidade) · fotos · **deslocamento** (Sim/Não + horários) · **início/fim do levantamento** · **almoço/pausa** · **técnico do levantamento** (o criador, fixo) · **acompanhante opcional com justificativa obrigatória** (06/08) · **data de referência** (fixada na criação).
- **`data` = data de REFERÊNCIA, fixada na criação (v683):** o campo `data` do pré-orçamento é a **data de referência do levantamento**, gravada **uma vez** no `novoPreorc` e **nunca recarimbada** no salvar/concluir/sincronizar. **Limitação honesta (não é garantia de "data realizada"):** no fluxo normal a criação acontece **durante a visita**, então a data bate com o dia do levantamento — mas em **registro tardio** (o técnico cria no dia seguinte) a data será a de **criação**, não a da visita. Informar uma visita **passada** de propósito dependerá, no futuro, de **campo de data editável** (ou outra fonte comprovada). *(Bug corrigido em 24–28/07, dois pontos: (a) `data` era `new Date()` reescrito a cada salvar/concluir — `tecnico.js` `poMontarPayload`/`concluirPreorc` — escorregando para o dia do sync; (b) o PDF da Edge Function `documentos` usava `criado_em`, **ignorando** `data` — corrigido para `emissaoPreorc(po)`: `data` → `criado_em` (fallback legado) → agora, em `supabase/functions/documentos/emissao.mjs`, com teste `emissao.test.mjs`. Caso real: Nº 14 do Maicon, feito 22/07, saiu 24/07 no PDF/jornada por ter sido re-sincronizado nesse dia.)* **Início/fim da visita alimentam a jornada do técnico** (`respostas.visita_inicio`/`visita_termino` lidos ao vivo com `pre_orcamentos.data`, sem view/segmento materializado) — por isso a data tem que ser imutável.
- **Regra: 1 técnico + acompanhante COM justificativa (06/08 — SW v696; substitui a regra "1 técnico" da v657).** Levantamento continua trabalho de uma pessoa — o **dono** (criador). Decisão de gestão de 06/08: o app permite **UM segundo técnico (acompanhante)**, e a presença dele **exige justificativa** (campo obrigatório validado no Concluir; rascunho salva sem). O acompanhante é **documental e de jornada**: entra em `respostas.tecnicos` (+ `respostas.acompanhante_justificativa`), aparece no PDF (linha "Justificativa do acompanhante" na seção Levantamento), conta na **Jornada** (participações) e no **almoço materializado** (0143) — mas **NÃO vira co-dono**: RLS/sync/edição seguem só do dono (`tecnico_id`), sem tabela de junção. *(Histórico: 26/06–19/07 houve seletor de equipe documental — v526; removido na v657 com a regra "1 técnico"; reintroduzido na v696 no formato acompanhante+justificativa.)*
...
**Conferência "sobreposição de horários entre RATs" (20/07 — Fase 1 do redesenho "passagem de bastão", `docs/redesenho-passagem-bastao.md`):** a `vw_alerta_sobreposicao` (migração 0122, molde da `vw_alerta_desloc_sem_volta`) lista **pares de RATs do MESMO técnico no MESMO dia já encerrado** cujos horários se cruzam, com o intervalo conflitante calculado. Banner âmbar na **Jornada** (com link pras duas RATs e a janela de cada uma) e card âmbar no **Painel** (janela de 14 dias; o item abre a Jornada do dia via `?d=`). Decisões: **só leitura** — não trava o técnico, não altera horário, não entra em desempenho nem faturamento; encostar (fim = início) **não** é sobreposição; intervalo aberto/inválido fica fora; sobreposição **pode ser legítima** ("saiu e voltou" — 5 dos 7 casos históricos), por isso alerta de conferência, nunca correção automática. **Pausa explica o cruzamento (05/08 — migração 0142):** quando o intervalo de conflito está **inteiramente dentro da pausa declarada** de uma das duas RATs (`respostas.pausa='Sim'` + janela), o par **nem entra no alerta** — é o técnico usando a pausa de um serviço pra registrar o outro (em produção: 14 pares → 2 no apply). Cobertura **parcial** da pausa continua alertando; sobreposição sem pausa (ex.: atendeu outro cliente no meio do serviço) continua alertando e é tratada com o "Revisado". O encerramento automático de participação (ledger/motor server-side, modal "manter também", editor de horário individual) foi **deliberadamente adiado** — decisão de 20/07 registrada no doc do redesenho. **Revisão persistida (05/08 — migração 0140, `sobreposicao_revisoes`):** botão **"Marcar verificada"** no item (Jornada e Painel) registra quem/quando conferiu, por **par de RATs** (ids ordenados — o mesmo par listado para 2 técnicos limpa junto, o fato conferido é um só); par revisado **sai do Painel** e vai pra seção de revisadas da Jornada (oculta por padrão atrás de "Mostrar N revisada(s)", cinza, com **Reabrir** = desfaz; a seção exibe só os **últimos 30 dias** — revisões mais antigas seguem no banco e valendo, só saem da tela). A revisão guarda a **janela do conflito** conferida: se os horários forem editados depois e o cruzamento mudar, o par **volta a pendente** sozinho. RLS: só admin/gestor_axis leem/escrevem a tabela.
...
- **Transições automáticas** (trigger `rat_inicia_tarefa` no banco, `INSERT`/`UPDATE` — cobre offline, acerta no sync). **Desde a 0130 o alvo é DERIVADO DO CONJUNTO de RATs da Tarefa** (`tarefa_status_alvo(tarefa)`), não da linha sincronizada — reprocessar snapshot velho dá o mesmo alvo (ordem-independente; matou o flapping *Em execução ↔ Em pausa* do caso 04853). O app **não escreve mais status direto** no sync (o antigo passo 4c saiu; só ficou o destravamento devolvida→concluída), e um guard no banco (`trg_tarefa_status_guard`) segura escrita defasada de app antigo (`em_execucao` com pausa aberta → mantém `em_pausa`):
  ...
  - RAT encerrada com **"Volta amanhã? = Não" + "vou voltar depois pra terminar"** (RAT mais recente) → Tarefa **Em execução → Em pausa** (nunca rebaixa status terminal/admin). O handoff **"o que falta / o que levar" segue obrigatório**. **RAT registrada SEM o checkpoint respondido** (ex.: gestão registrou pelo portal, fluxo que não tem a pergunta) **não decide continuidade** — vale o **último checkpoint respondido** da Tarefa (0135; espelha `passagemAberta()` do app — caso 04778: o backfill da 0130 atropelou uma pausa vigente porque a RAT mais recente tinha sido registrada pelo portal sem checkpoint).
...
- **Pausa manual da gestão — durável (0136):** a gestão pode pausar pelo portal (select de status → modal com **motivo opcional**, registrado na auditoria como "manual da gestão"). Grava **marcador** `pausa_manual_em/por/motivo` na Tarefa (não é status novo; **nunca escreve na RAT do técnico** — o documento é dele). Enquanto o marcador viver, **nenhuma escrita automática rebaixa** `Em pausa → Em execução` (edição de RAT, sync atrasado, backfill — o guard segura). Libera de duas formas: **(a) despausa explícita** no portal (escolher outro status limpa o marcador no mesmo update; app antigo não envia a coluna → escrita defasada segue bloqueada); **(b) retomada real** — RAT **criada depois** da pausa (`criado_em >`, separa RAT nova de reabertura administrativa) **e** com **dia declarado ≥ dia da pausa** (fuso America/São_Paulo; separa retomada de campo de **documentação retroativa** — registro tardio de trabalho pré-pausa, caso 04790, **não** derruba a pausa). Status terminal/admin nunca é bloqueado e limpa o marcador. Pílula/cabeçalho mostram "pela gestão · nome · data" (tooltip com o motivo). Trade-off aceito: técnico que abre RAT nova **por engano** despausa — auditoria + re-pausa cobrem (status segue o campo). Teste: `supabase/tests/teste_0136_pausa_manual.sql`.
...
- **Uma Tarefa = o serviço inteiro** (guarda-chuva); **uma RAT por dia trabalhado** (filha), numeradas 04750/1 … 04750/N. Cada RAT diária é o **diário do dia**: trabalho feito, tempo por técnico, material usado, fotos. **Rede no servidor (F3, migração 0146 — decisão 08/26):** índice único parcial **uma RAT por (tarefa, dia, TÉCNICO)** — improdutiva fora (visita fechada à parte + nova RAT do dia é legítimo); escopo por técnico porque o RLS de `rats` não dá convergência entre donos (unique global encalharia coautor). Duplicata offline barrada no sync vira **bloqueio nomeado sem loop** (espelho do F19): nada apagado, rótulo na lista, retry só manual. Sem `client_uuid` determinístico (UUIDv5): colidiria com improdutiva→nova RAT do dia (sobrescrita silenciosa no upsert); a convergência online já existe via pull (`topUpRats90` + `ratDoDiaDe`).
...
**Visões de RAT, Deslocamento e Jornada no portal (06/26; Jornada revisada 08/26).** Além da aba dentro da Tarefa, o portal tem páginas próprias:
- **RATs** — alterna **Calendário** (visão mensal, agrupa pela **data da RAT**) e **Lista** (busca no banco inteiro). Filtros: cliente (combobox), técnicos (multi, mostra co-responsáveis), busca livre. O chip mostra o **Nº da RAT** `{tarefa}/{seq}`. Atalho direto pra aba RATs da Tarefa. **`data_tarefa` segue a Data declarada da RAT** (`respostas.data`) por trigger no servidor (migração 0144, decisão 08/26): RAT criada retroativamente ficava invisível no calendário quando a criação caía em outro mês (caso 4959/01 — a busca do mês usa `data_tarefa`, o chip usa a declarada); a declarada é a fonte, o carimbo re-deriva em todo insert/update (declarada vazia mantém o carimbo da criação).
...
- **Jornada (composição vigente, 08/26)** — conferências (**deslocamento sem volta** e **sobreposição de horários entre RATs**, com "Revisado" persistido/0140) · filtro **técnico ("Todos os técnicos" padrão — filtra a tabela do dia) + data** (deep-link `?d=` do Painel) · **Horas do dia por técnico** (§8: participações em RATs/Deslocamentos/Pré-orçamentos − almoço único; avatar com foto em TODAS as tabelas da página) · **Pernoites por técnico** · **Deslocamento por técnico (período)** · **Conciliação de almoço SR × Tangerino**. **Decisão 08/26:** os cards de totais por tipo, o quadro "Horas de trabalho por cliente" e a "Linha do tempo" **saíram** — liam `jornada_segmentos` (rastreio MANUAL do app, quase nunca preenchido) e viviam zerados ao lado da tabela com horas reais, parecendo dado perdido. A visão volta quando o modo "dia contínuo" (§10.1) for construído de verdade; a tabela `jornada_segmentos` e a tela "Jornada" do app seguem existindo (nenhum dado apagado).
...
- **Travas também no SERVIDOR (08/26 — migração 0147, F2/F4 do doc de robustez):** as regras deixam de viver só no JS (burlável por API direta com o JWT do técnico). Triggers no Postgres: **concluir a Tarefa exige RAT registrada e barra retorno em aberto** (`validar_conclusao_tarefa`; **admin/gestor força com ciência** — e o portal acusa "Concluída sem RAT registrada · verificar" no card Situação, alerta F12); **RAT tem validação mínima** (`validar_rat`): improdutiva exige motivo · horas presentes legíveis · ordem invertida real (> 12h de distância de relógio; madrugada legítima passa) · término no futuro só pra RAT de hoje (BR, tolerância 15 min, auto-cura no retry). **Escopo deliberadamente conservador** — falso-reject encalha RAT no aparelho; a cronologia fina (pausa/desloc×execução) segue só no app. Regra da casa registrada: isenção por papel ⇒ trigger INVOKER; agregado além-RLS ⇒ helper DEFINER (`fn_tarefa_tem_rat_registrada`).
- **Lista de RATs da tarefa no app mescla aparelho + SERVIDOR (08/26, F13):** RAT que só existe no servidor (outro aparelho ainda não puxado; cópia local excluída) aparece como cartão de **leitura** ("No servidor", `/NN` e situação) em vez de sumir — antes a tela "parecia sem RAT". Offline cai pro local; o RLS decide o que o servidor devolve (coautor segue invisível — limitação de policy registrada no F3).
...
- **Atualização servidor→aparelho em camadas (07/26, migração 0137):** o caminho de descida tem 3 camadas, da mais rápida pra rede de segurança: (1) **Realtime** — o app assina `postgres_changes` das tabelas do `SYNC_MAP` (`rats`, `pre_orcamentos`, `deslocamentos`, `jornada_segmentos`) + `sync_tombstones`, e qualquer sinal dispara um **pull delta** com debounce de 400 ms; `tarefas` e `tarefa_tecnicos` têm sinal próprio (`onTarefasSignal`) que **re-busca e re-renderiza** a tela atual (Home/lista/detalhe) — tarefas não passam pelo pull (vêm de SELECT direto com RLS). O handler **nunca re-renderiza por cima de formulário aberto** (RAT/pré-orçamento em edição ficam fora, mesma regra do `onSyncChanged`). (2) **Eventos do aparelho** — reconexão (`online`) e volta pra aba (`visibilitychange`) puxam na hora. (3) **Timer de segurança** — `pullChanges` a cada 2 min (se o websocket cair, nada fica pra trás por mais que isso). **Armadilha que motivou a 0137:** o app assinava o Realtime dessas tabelas desde sempre, mas `rats`/`pre_orcamentos` **não estavam na publication `supabase_realtime`** — o sinal nunca chegava e a atualização de RATs caía nos 2 min do timer. Regra: **tabela nova no `SYNC_MAP` (ou assinada no canal) tem que entrar na publication** (`alter publication supabase_realtime add table ...`), senão o realtime dela é silenciosamente morto. RLS continua valendo no Realtime (só chega evento de linha que o assinante pode ler).
...
- **Usuário desativado — desligamento (07/26, PRs #139–#141):** desligar alguém = **desmarcar "Ativo" no Portal** (`usuarios.ativo=false`; o Portal também bane o login no Auth). **NUNCA remover as linhas de `portal_acessos` de quem tem histórico** — `sr_usuarios()` só devolve quem tem acesso `service_report`, então remover o acesso apaga o ex-funcionário de TODAS as telas do SR (causa real do sumiço de Charles/Max em 07/26; acessos recriados via SQL, login segue bloqueado pelo ban + `ativo=false`). Regra no front: **listas de referência usam TODOS os usuários** (resolução de nomes/avatares do histórico — RATs, tarefas, deslocamentos, jornada, desempenho, calendários); **pickers de trabalho novo filtram `ativo !== false`** (ninguém atribui RAT/tarefa/deslocamento novo a ex-funcionário; exceção: inativo já responsável/participante de registro antigo continua listado nele, não some ao salvar); dropdowns de **filtro de consulta** mostram inativos com rótulo "— inativo". Visual: classe **`.u-inativo`** (grayscale + opacidade, `design-system-admin.css`) em chip/avatar/nome onde o inativo aparece — participante de RAT, responsável de tarefa (chips, coluna da lista e cabeçalho do modal), a bordo de deslocamento (lista/detalhe/calendário), jornada e desempenho; ativos seguem coloridos. `ponto-vinculos.js` fica fora da regra (tela operacional do Tangerino, já trata inativos com `mostrarInativosSR`).
...
- **Usuários desativados visíveis no histórico (07/26, PRs #139–#141):** telas deixaram de descartar inativos do `sr_usuarios` — nomes/avatares resolvem em RATs, tarefas, deslocamentos, jornada, desempenho e calendários; chip/avatar/nome cinza via `.u-inativo` ("Inativo"); filtros com "— inativo"; pickers de atribuição só com ativos (§12).
...
2. **E-mail da RAT concluída → adm@tsrv.com.br — ✅ FEITO (08/26, SW v714):** a Edge `documentos` ganhou a action **`rat_registrada`** — quando a RAT do dia é ENCERRADA (confirmada no sync com status `registrado`), a administração recebe **e-mail HTML com o conteúdo completo** (nº `{tarefa}/{seq}`, cliente, técnicos da RAT, data, execução/almoço/deslocamentos, tempo, orientação da OS, serviço executado, observações, pendência, materiais) **+ link direto pro portal**. **Decisão de escopo: SEM PDF anexo** — o PDF oficial da RAT nasce do motor compartilhado no cliente (duplicá-lo na Edge criaria um 2º formato); anexá-lo é evolução futura se a administração pedir. Idempotente por `rats.email_adm_em` (migração 0150, com **backfill sentinela** das registradas existentes — só encerramento NOVO notifica; re-sync/edição de RAT antiga não dispara e-mail atrasado). Gatilho: fire-and-forget no `sync.js` pós-confirmação (nunca afeta o sync; a Edge deduplica). Destinatário via env `RAT_EMAIL_TO` (default adm@tsrv.com.br).
...
12. **Índice de assertividade do técnico — ✅ FEITO (08/26, SW v713):** seção **"Assertividade de preenchimento"** na página Desempenho (admin/gestor), por mês: % das RATs do técnico (data declarada) **sem correção da gestão por `esquecimento_tecnico`** — o motivo já sai categorizado do editor auditado (`rat_edicoes`). Completação/correção de texto/mudança de processo **não contam contra** (aparecem como "outras edições"); `sync_app*` (re-sync do aparelho) fica fora. **Régua de atribuição: a RAT conta pra TODOS os técnicos dela (`rat_tecnicos` ∪ titular) — a mesma do placar (0097)**; a régua "só titular" da 1ª versão escondia quem nunca é o digitador no modelo colaborativo (agosto mostrava 3 técnicos com 6 em campo; Max Macedo aparecia com 1 RAT/100% quando na régua certa tem 27 RATs/67% em julho). Tabela com avatar, campos mais esquecidos em chips, faixa de cor (≥90% verde · ≥70% âmbar · abaixo vermelho) e **drill por linha** (quando · RAT · campo · quem corrigiu · "preenchida por X" quando a RAT não é do próprio técnico). Validado contra produção por SQL (julho: 8 técnicos — Pablo 93% em 46 · Alessandro 88% em 25 · Max 67% em 27 · Marcelo 62%; agosto parcial: 6 técnicos, 100%).
