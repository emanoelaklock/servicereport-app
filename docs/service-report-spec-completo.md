...
14. **Conflito de material em RAT colaborativa (plano pronto — projeto próprio):** dois técnicos lançando material **offline na MESMA RAT** (cada aparelho com ids próprios) → o `enviarRat` insere os dois conjuntos e **DUPLICA** (infla faturamento). Plano aprovado (**dono-é-a-RAT**, opção 1, respeita §8 "nunca soma em silêncio"): `materiais.created_by`/`device_id` + **trigger** que marca `rats.material_conflito` quando há **≥2 autores**; **gate de faturamento** (não soma, pede resolver); **resolução pelo editor de RAT** (admin remove o conjunto duplicado, auditado/restaurável). **Caso isolado hoje** (só a 04765, já limpa) e a **hidratação ao reabrir** (24/06) já mitiga o caso **sequencial**; sobra o **offline-concorrente**. 5 fases, mexe em **sync + faturamento + banco** → sessão dedicada, por fases testadas. **Teste crítico:** re-sync do **MESMO autor** NÃO pode acusar conflito falso (senão enche o admin de selo falso).
