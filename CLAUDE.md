...
- **RAT colaborativa:** uma RAT por `(tarefa, dia)`. O `client_uuid` é **UUID v4 aleatório** (`crypto.randomUUID`, `db-local.js`) — **decisão 08/26 (F3): NÃO trocar por UUIDv5 determinístico** (colide com "improdutiva + nova RAT do dia" e sobrescreveria em silêncio). A regra "1 por dia" é client-side (`ratDoDiaDe` reusa; pull `topUpRats90` converge aparelhos do mesmo técnico) **+ rede no servidor**: índice único parcial `uq_rats_tarefa_dia_tecnico` **(tarefa, dia, TÉCNICO)** — por técnico porque o RLS de `rats` não dá convergência entre donos; improdutiva fora (0146). Duplicata barrada vira bloqueio nomeado no sync (sem loop, nada apagado). Sub-tabelas com **`created_by`**; merge por **união**; conflito (dois lançam o mesmo) é **marcado pro admin, nunca somado em silêncio**.
